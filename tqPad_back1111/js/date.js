    ...
    $.fn.date = function (options,Ycallback,Ncallback) {   
        ...
        if(!opts.show){
            ...
            that.bind(opts.event,function () {
                ...
                extendOptions(); //显示控件
                that.blur();
				var screen_w=$(window).width();		//获取屏幕的宽度
				var screen_h=$(window).height();	//获取屏幕的高度
				$('body,html').addClass('kf_screen');
				$('.kf_screen').width(screen_w).height(screen_h);
                if(datetime){
                    showdatetime();
                    refreshTime();
                }
	...
	function resetIndex(){
        ...
        function bindButton(){
            ...
            $("#dateconfirm").unbind('click').click(function () {	
				...
				$('body,html').removeClass('kf_screen').height('auto').width('auto');
               if(datetime){
        ...
        function createDAY_UL(){
             $("#daywrapper ul").html("");
            	var str="<li>&nbsp;</li>";
				var nian=$("#yearwrapper ul li:eq("+indexY+")").html().substr(0,$("#yearwrapper ul li:eq("+indexY+")").html().length-1);
				var yue=$("#monthwrapper ul li:eq("+indexM+")").html().substr(0,$("#monthwrapper ul li:eq("+indexM+")").html().length-1);
				if(yue==4||yue==6||yue==11){opts.endday=30;}
				else if(yue==2){
					if(nian%100==0){
						if(nian%4==0){opts.endday=29;}
						else{opts.endday=28;}
					}
					else if(nian%100!=0&&nian%4==0){opts.endday=29;}	
					else{opts.endday=28;}
				}
