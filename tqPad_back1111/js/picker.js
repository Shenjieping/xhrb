...
(function(factory) {
	...
	function PickerConstructor(ELEMENT, NAME, COMPONENT, OPTIONS) {
					...
					if(dontGiveFocus !== false) {
						...
						$document.on("click." + STATE.id + " focusin." + STATE.id, function(event) {
							...
							if(keycode == 27) {
								...
								if(target == ELEMENT && (keycodeToMove || keycode == 13)) {
		...
		function prepareElementHidden() {
			...
			P._hidden = $("<input " + "type=hidden " + 'name="' + name + '"' + 'id="' + id + '"' + ($ELEMENT.data("value") || ELEMENT.value ? ' value="' + P.get("select", SETTINGS.formatSubmit) + '"' : "") + ">")[0];
